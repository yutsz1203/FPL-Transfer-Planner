...
def get_myteam_season_stats(gw):
    ...
    file_path = MYTEAM_RESULTS_DIR / "myteam_currentseason_stats.csv"
...
def get_myteam_lastn_stats(gw, n):
    ...
    player_ids = [player["element"] for player in players]
    new_df = players_df[players_df["Player ID"].isin(player_ids)]
    file_path = MYTEAM_RESULTS_DIR / f"myteam_last{n}games_stats.csv"
    new_df.to_csv(file_path, index=False)
    print(new_df.to_string(index=False))
    print(
...
if __name__ == "__main__":
    ...
    n = int(input("Enter n, the number of gameweeks used in calculation: "))
