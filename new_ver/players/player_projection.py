...
def get_next_gameweek(fixtures, player_df, team_df, type, gw, side_adjustment=True):
    ...
    for player in players:
        ...
        opponent = fixtures[fixtures["team"] ==
                            team_name][gameweek].values[0][:-3]
        ...
        if side_adjustment:
            if side == "H":
                ...
                team_Oi = team_df[team_df["team"]
                                  == team_name]["h_Oi"].values[0]
                team_Di = team_df[team_df["team"]
                                  == team_name]["h_Di"].values[0]
                opponent_Oi = team_df[team_df["team"]
                                      == opponent]["a_Oi"].values[0]
                opponent_Di = team_df[team_df["team"]
                                      == opponent]["a_Di"].values[0]
            else:
                xGI = player["a_xGI/Game"]
                xGc = player["a_xGc/Game"]
                team_Oi = team_df[team_df["team"]
                                  == team_name]["a_Oi"].values[0]
                team_Di = team_df[team_df["team"]
                                  == team_name]["a_Di"].values[0]
                opponent_Oi = team_df[team_df["team"]
                                      == opponent]["h_Oi"].values[0]
                opponent_Di = team_df[team_df["team"]
                                      == opponent]["h_Di"].values[0]
        else:
            xGI = player["xGI/Game"]
            xGc = player["xGc/Game"]
    ...
    next_gameweek_df.sort_values(
        by=["projected GI"], ascending=False, inplace=True)
    next_gameweek_df.to_csv(
        f"players/projection/next_gameweek_{type}.csv", index=False)
...
def get_next_n_gameweek(
    ...
    print(
        f"Projection gw{gw}-gw{gw+(n-1)} for all teams using {type} stats...")
    print("*" * 90)
    ...
    for player in players:
        ...
        nextn_gws = [team_fixtures[f"GW{i}"].values[0]
                     for i in range(gw, gw + n)]
        opponents = []
        (
            total_team_Oi,
            total_team_Di,
        ...
        for oppo in nextn_gws:
            ...
            if side_adjustment:
                ...
                total_team_Oi += team_df[team_df["team"]
                                         == team_name]["Oi"].values[0]
                total_team_Di += team_df[team_df["team"]
                                         == team_name]["Di"].values[0]
    ...
    next_n_gameweek_df["projected_net_goals"] = round(
        next_n_gameweek_df["projected_GI"] -
        next_n_gameweek_df["projected_Gc"], 2
    )
    ...
    next_n_gameweek_df.sort_values(
        by=["projected_GI"], ascending=False, inplace=True)
    next_n_gameweek_df.to_csv(
        f"players/projection/next{n}gameweeks_{type}.csv", index=False
    )
...
if __name__ == "__main__":
    ...
    player_current_season = pd.read_csv(
        "players/results/players_currentseason.csv")
    ...
    player_lastngames = pd.read_csv(
        f"players/results/players_last{n}games.csv")
